...
export const portalRouter = router({
  mine: studentProcedure.query(async ({ ctx }) => {
    ...
    const results = await db.select({ result: assessmentResults, title: assessments.title, assessmentType: assessments.assessmentType, totalScore: assessments.totalScore }).from(assessmentResults).innerJoin(assessments, eq(assessmentResults.assessmentId, assessments.id)).where(and(eq(assessmentResults.studentId, profile.id), isNull(assessments.deletedAt))).orderBy(desc(assessmentResults.createdAt));
