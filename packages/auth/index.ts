import { AXIOS_TIMEOUT_MS, COOKIE_NAME, ONE_YEAR_MS, decodeOAuthState } from "@blush/shared/const";
import { ForbiddenError } from "@blush/shared";
import { ENV } from "@blush/env";
import * as db from "@blush/db";
import type { User } from "@blush/db/schema";
...
class SDKServer {
  ...
  /**
   * Authenticate an incoming request from any Fetch-API `Request`
   * (Next.js Route Handlers and Server Components both hand you one via
   * `NextRequest`, which extends `Request`).
   */
