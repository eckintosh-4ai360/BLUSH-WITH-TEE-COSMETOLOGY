...
export default function AdmissionsPage() {
  const { data: courses = [] } = trpc.platform.content.courses.useQuery();
  const utils = trpc.useUtils();
  const submit = trpc.platform.admissions.submit.useMutation();
  const upload = trpc.platform.admissions.uploadDocument.useMutation();
  const [lookupInput, setLookupInput] = useState<{ reference: string; email: string } | null>(null);
  const lookup = trpc.platform.admissions.lookup.useQuery(lookupInput ?? { reference: "APP-000000", email: "placeholder@example.com" }, { enabled: Boolean(lookupInput) });
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      utils.platform.admin.dashboard.invalidate();
