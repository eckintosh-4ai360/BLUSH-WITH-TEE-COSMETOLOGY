...
const ORDER_STEPS = ["new", "confirmed", "processing", "ready", "shipped", "delivered"] as const;

function OrderStatusTracker({ status }: { status: string }) {
  if (status === "cancelled") {
    return <p className="mt-1 text-xs font-bold uppercase tracking-[.1em] text-[#e01a4f]">Order cancelled</p>;
  }
...
export default function StorePage() {
  ...
  const orderLookup = trpc.store.lookupOrder.useQuery(
    lookupInput ?? { orderNumber: "ORD-000000", email: "placeholder@example.com" },
    { enabled: Boolean(lookupInput) }
  );
  const utils = trpc.useUtils();
  const addItem = trpc.store.addItem.useMutation({
    onSuccess: () => utils.store.cart.invalidate({ sessionToken: sessionToken ?? "" }),
  });
  ...
  async function submitCheckout(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice(`Order ${result.orderNumber} placed successfully. Our team will verify and prepare your beauty supplies.`);
    } catch (error) {
        ...
        <div className="grid gap-12 xl:grid-cols-[1fr_360px]">
          <section>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#692156]">
              Equip your craft with top-tier beauty tools, student training kits, skincare products, and hair styling formulas curated by Blush With Tee.
            </p>
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2">
              {isLoading ? (
                [1, 2].map(id => (
                  <div className="h-96 animate-pulse rounded-3xl bg-white/70 border border-[#8f0d6b]/10" key={id} />
                ))
              ) : (
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <h2 className="mt-2 font-serif text-2xl font-bold text-[#8f0d6b]">{product.name}</h2>
                        </div>
                        <p className="font-serif text-xl font-bold text-[#8f0d6b]">
                          GHS {product.sellingPrice.toFixed(2)}
                        </p>
                      </div>
                      <p className="mt-3 text-sm leading-6 text-[#6a2557]">{product.description}</p>
                      <div className="mt-6 flex items-center justify-between">
                        <span className="text-xs font-semibold text-[#8f0d6b]">
                          {product.quantityOnHand > 0 ? `${product.quantityOnHand} available` : "Out of stock"}
                        </span>
                        <Button
                          disabled={product.quantityOnHand === 0 || addItem.isPending}
                          onClick={() =>
                            addItem.mutate({
                              sessionToken: sessionToken ?? "",
          ...
          <aside className="h-fit rounded-[2.25rem] border border-[#8f0d6b]/15 bg-white/95 p-7 shadow-[0_16px_40px_rgba(143,13,107,.08)] xl:sticky xl:top-28">
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-3xl font-bold text-[#8f0d6b]">Your Bag</h2>
            ...
            <div className="mt-6 grid gap-4">
              {items.length === 0 && (
                <p className="rounded-2xl bg-[#fdf2f9] p-5 text-sm leading-6 text-[#8f0d6b]">
                  Your bag is currently empty. Browse our beauty products and training essentials.
                </p>
              )}
              {items.map(item => (
                <div key={item.cartItemId} className="border-b border-[#8f0d6b]/10 pb-4">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <div className="flex items-center gap-1 rounded-full border border-[#8f0d6b]/20 px-1 bg-white">
                      ...
                      <span className="w-6 text-center text-xs font-bold text-[#8f0d6b]">{item.quantity}</span>
                      <button
                        onClick={() =>
                          updateItem.mutate({
                            sessionToken: sessionToken ?? "",
                            cartItemId: item.cartItemId,
                            quantity: Math.min(item.quantity + 1, item.quantityOnHand),
                          })
                        }
                        className="grid h-7 w-7 place-items-center rounded-full hover:bg-[#faeaf6]"
                      >
                    ...
                    <span className="text-sm font-bold text-[#8f0d6b]">GHS {item.lineTotal.toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {checkoutOpen && (
              <form onSubmit={submitCheckout} className="mt-6 grid gap-3 border-t border-[#8f0d6b]/15 pt-5">
                <p className="text-sm font-bold text-[#8f0d6b]">Order Delivery Details</p>
                <input required className="soft-input" name="customerName" placeholder="Full Name" />
                <input required type="email" className="soft-input" name="customerEmail" placeholder="Email Address" />
                <input required className="soft-input" name="customerPhone" placeholder="Phone Number" />
                <textarea className="soft-input min-h-20" name="deliveryAddress" placeholder="Campus / Delivery Address" />
                <Button
                  disabled={checkout.isPending}
                  type="submit"
                  className="rounded-full bg-gradient-to-r from-[#fe00b6] to-[#8f0d6b] py-5 text-sm font-bold text-white shadow-md"
                >
                  {checkout.isPending ? "Placing Order…" : "Confirm & Place Order"}
                </Button>
              </form>
            )}
            ...
            >
              <p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#8f0d6b]">Track An Order</p>
              <input required name="orderNumber" placeholder="Order Reference (e.g. ORD-1002)" className="soft-input" />
              <input required type="email" name="lookupEmail" placeholder="Customer Email" className="soft-input" />
              <Button type="submit" variant="outline" className="rounded-full border-[#8f0d6b]/25 bg-white text-[#8f0d6b] hover:bg-[#faeaf6]">
                Find Order
              </Button>
            </form>
            ...
            {orderLookup.data && (
              <div className="mt-4 rounded-2xl bg-[#faeaf6] p-4 text-xs leading-6 text-[#8f0d6b] border border-[#fe00b6]/30">
                ...
                <br />
                Status: <span className="font-bold uppercase">{orderLookup.data.fulfillmentStatus.replaceAll("_", " ")}</span> · Payment:{" "}
                <span className="font-semibold">{orderLookup.data.paymentStatus}</span>
                <OrderStatusTracker status={orderLookup.data.fulfillmentStatus} />
              </div>
            )}
            {orderLookup.error && <p className="mt-3 text-xs font-semibold text-[#e01a4f]">{orderLookup.error.message}</p>}
          </aside>
        </div>
      </main>
    </PublicShell>
