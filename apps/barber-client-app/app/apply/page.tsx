...
function ApplicationStatusTracker({ status }: { status: string }) {
  if (status === "rejected") return <p className="mt-2 text-xs font-semibold uppercase tracking-[.1em] text-[#a14d65]">Application not approved</p>;
  const stepIndex = APPLICATION_STEPS.indexOf(status as (typeof APPLICATION_STEPS)[number]);
  return <div className="mt-3 flex items-center gap-1">{APPLICATION_STEPS.map((step, index) => <span key={step} className={`h-1.5 flex-1 rounded-full ${index <= stepIndex ? "bg-[#5f8c6d]" : "bg-[#e3e6e2]"}`} title={step.replaceAll("_", " ")} />)}</div>;
}
